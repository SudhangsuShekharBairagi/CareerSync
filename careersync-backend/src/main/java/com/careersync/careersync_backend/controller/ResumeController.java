...
public class ResumeController {
    ...
    public ResponseEntity<ResumeAnalysis> upload(
        ...
        if (file == null || file.isEmpty()) {
            throw new IllegalArgumentException("Please upload a PDF resume file.");
        }

        List<String> data = resumeService.uploadResume(file);
        ResumeAnalysis analysis = geminiService.analyzeResume(data.get(1));
        resumeService.saveAnalyzeResume(analysis, data.get(0));
    ...
    public ResponseEntity<ResumeAnalysis> getResumeById(@PathVariable Long id) {
        ...
        ResumeAnalysis response = resumeService.getResumeById(id);
