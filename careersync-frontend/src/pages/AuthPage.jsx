  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        await register({ username: name, email, password, otp })
      }
      navigate('/resume')
    } catch (err) {
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {!isLogin && !otpSent && (
              <button
                ...
                disabled={otpSending}
                style={{
                  background: 'transparent', color: t.primaryCont,
                  border: `1px solid ${t.primaryCont}`, borderRadius: 8,
                  padding: '13px 20px', fontSize: 13, fontWeight: 600,
                  cursor: otpSending ? 'default' : 'pointer', width: '100%',
                  ...fontInter, transition: 'background 0.15s', opacity: otpSending ? 0.6 : 1,
                }}
              >
            ...
            {!isLogin && otpSent && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <input id="input-otp" type="text" inputMode="numeric" placeholder="6-digit code" required style={s.input}
                    maxLength={6}
                    value={otp}
                  ...
                  <button
                    ...
                    disabled={otpSending}
                    style={{
                      flexShrink: 0, background: 'transparent', color: t.primaryCont,
                      border: `1px solid ${t.primaryCont}`, borderRadius: 8,
                      padding: '0 14px', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap',
                      cursor: otpSending ? 'default' : 'pointer', ...fontInter,
                      transition: 'background 0.15s', opacity: otpSending ? 0.6 : 1,
                    }}
                  >
                    {otpSending ? '...' : 'Resend'}
                  </button>
            ...
            <button id="btn-submit-auth" type="submit" disabled={submitting || (!isLogin && !otpSent)} style={{
              background: '#fff', color: t.bg, border: 'none',
              padding: '13px 20px', borderRadius: 8, fontSize: 14, fontWeight: 600,
              cursor: (submitting || (!isLogin && !otpSent)) ? 'default' : 'pointer', width: '100%',
              ...fontInter, marginTop: 4, opacity: (submitting || (!isLogin && !otpSent)) ? 0.6 : 1,
